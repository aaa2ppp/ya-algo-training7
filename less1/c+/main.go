...
// Для подключения нужно купить карточку, позволяющую пользоваться интернетом определенное
// количество секунд. При этом компания продает карточки стоимостью 1, 2, 4, ..., 2^30 рублей
// на a0, a1, a2, ..., a30 секунд соответственно.
// Родители разрешили Пете пользоваться интернетом M секунд. Определите, за какую наименьшую
// сумму он сможет купить карточки, которые позволят ему пользоваться интернетом не менее M
// секунд. Естественно, Петя может купить как карточки различного достоинства, так и несколько
// карточек одного достоинства.

const maxCardTypes = 31
...
// solve возвращает наименьшую сумму денег, которую Пете придется потратить.
func solve(m int, aa []int) int {

	// формируем список карточек
	cards := make([]card, 0, len(aa))
	for i := range aa {
	...
	// сортируем карточки по эффективности time/price
	slices.SortFunc(cards, func(a, b card) int {
		return b.time*a.price - a.time*b.price
	})
	...
	var (
		rem      = m           // остаток непокрытого карточками времени
		cost     = 0           // стоимость уже покрытого времени
		minTotal = math.MaxInt // ищем минимальную стоимость всего времени
	)

	for _, c := range cards {

		// по максимуму, покрываем не более оставшегося время-1 самыми эффективными карточками
		n := (rem - 1) / c.time
		rem -= n * c.time
		cost += n * c.price

		// пробуем докупить еще одну карточку (она гарантированно покроет остаток)
		minTotal = min(minTotal, cost+c.price)

		if debugEnable {
			log.Println(c, rem, cost, minTotal)
		}
	}
