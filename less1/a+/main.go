...
func solve(xx, yy []int) (int, []int) {
	...
	slices.SortFunc(oxx, func(a, b [2]int) int {
		return a[1] - b[1]
	})

	slices.SortFunc(oyy, func(a, b [2]int) int {
		return a[1] - b[1]
	})
	...
	for i < n && j < m {
		if oxx[i][1] < oyy[j][1] {
			ans[oxx[i][0]] = oyy[j][0] + 1 // to 1-indexing
			i++
		}
		j++
	}
...
func run(in io.Reader, out io.Writer) {
	...
	if err := scanInts(sc, yy); err != nil {
		panic(err)
	}

	k, ans := solve(xx, yy)

	writeInt(bw, k, defaultWriteOpts())
