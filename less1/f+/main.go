...
func solve(maxWeight int, weights, costs []int) []int {
	type dpItem struct {
		prev int
		cost int
		idx  int
	...
	n := len(weights)
	dp := makeMatrix[dpItem](n+1, maxWeight+1)
	...
	for i, w := range weights {
		idx := i + 1 // idx to 1-indexing
		copy(dp[idx], dp[idx-1])
		dpRow := dp[idx]

		c := costs[i]
		top = min(top+w, maxWeight)

		for j := top; j >= w; j-- {
			if dpRow[j-w].cost != -1 {
				if cur := dpRow[j-w].cost + c; cur > dpRow[j].cost {
					dpRow[j] = dpItem{
						prev: j - w,
						cost: cur,
						idx:  idx,
	...
	var ans []int
	for i, j := n, maxCostWeight; j > 0; i, j = dp[i][j].idx-1, dp[i][j].prev {
		ans = append(ans, dp[i][j].idx)
	}
