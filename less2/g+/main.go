...
func updateLeaf(tree []item, i int, val int32) {
	if val == 0 {
		tree[i] = item{1, 1, 1, 1}
	} else {
		tree[i] = item{len: 1}
...
func updateParent(tree []item, i int) {
	a, b := tree[i*2+1], tree[i*2+2]
	tree[i] = makeParent(a, b)
}

func makeParent(a, b item) item {
	x := item{
		len:    a.len + b.len,
		prefix: a.prefix,
		suffix: b.suffix,
...
func query(tree []item, ql, qr int) int {
	...
	dfs = func(i, l, r, ql, qr int) item {
		...
		// если интервал не прерасекается с моим
		if qr <= l || r <= ql {
			return item{}
		}
		...
		// иначе спросим у детей
		m := (l + r) / 2
		a := dfs(i*2+1, l, m, ql, qr)
		b := dfs(i*2+2, m, r, ql, qr)
		return makeParent(a, b)
	}

	n := (len(tree) + 1) / 2
	ans := dfs(0, 0, n, ql, qr)
	return int(max(ans.prefix, ans.middle, ans.suffix))
}
...
func run(in io.Reader, out io.Writer) {
	...
	for i := 0; i < m; i++ {
		...
		case "QUERY":
			...
			ans := query(tree, l-1, r)
			if debugEnable {
				log.Printf("q %d %d -> %d", l, r, ans)
			}
