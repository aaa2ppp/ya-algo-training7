...
func update(tree []int64, ql, qr int, add int64) {
	n := (len(tree) + 1) / 2
...
func query(tree []int64, idx int) int64 {
	n := (len(tree) + 1) / 2
	target := idx + n - 1
	...
	dfs = func(i, l, r int) int64 {
		...
		if m := (l + r) / 2; idx < m {
			return dfs(i*2+1, l, m) + tree[i]
		} else {
			return dfs(i*2+2, m, r) + tree[i]
		}
	}
