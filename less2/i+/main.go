...
func precalc(nums []int64) []item {
	...
	for i := n - 1 + len(nums); i < len(tree); i++ {
		tree[i].val = math.MinInt64
	}

	for i := n - 2; i >= 0; i-- {
		a, b := tree[i*2+1], tree[i*2+2]
		tree[i].val = max(a.val, b.val)
	}

	return tree
}

func update(tree []item, ql, qr int, add int64) {
	n := (len(tree) + 1) / 2
	...
	dfs = func(i, l, r int) bool {
		...
		if aUpd || bUpd {
			a, b := tree[i*2+1], tree[i*2+2]
			tree[i].val = max(a.val, b.val) + tree[i].add
			return true
		}
...
func query(tree []item, ql, qr int) int64 {
	n := (len(tree) + 1) / 2

	var dfs func(i, l, r int) int64

	dfs = func(i, l, r int) int64 {
		if debugEnable {
			log.Println("dfs:", i, l, r, ql, qr)
		}
		...
		aVal := dfs(i*2+1, l, m)
		bVal := dfs(i*2+2, m, r)
		return max(aVal, bVal) + tree[i].add
	}
...
func run(in io.Reader, out io.Writer) {
	...
	for i := 0; i < m; i++ {
		...
		case "m":
			...
			if debugEnable {
				log.Printf("m %d %d -> %d", l, r, ans)
			}
			writeInt(bw, ans, writeOpts{end: ' '})
		case "a":
			l, r, v, err := scanThreeInt(sc)
			if err != nil {
				panic(err)
			}
			// делаем из закрытого 1-base интервала
			// открытый 0-base интервал [l, r)
			update(tree, l-1, r, int64(v))
			if debugEnable {
				log.Printf("a %d %d %d -> %v", l, r, v, tree)
			}
		default:
