...
func query(tree []int32, ql, qr int) int32 {

	// первые три числа определяют узел: индекс и контролируемый интервал
	var dfs func(i, l, r, ql, qr int) int32

	dfs = func(i, l, r, ql, qr int) int32 {
		if debugEnable {
		...
		m := (l + r) / 2
		return max(
			dfs(i*2+1, l, m, ql, qr), // спросим у левого ребенка
			dfs(i*2+2, m, r, ql, qr), // спросим у правого ребенка
		)
	}

	n := (len(tree) + 1) / 2
	return dfs(0, 0, n, ql, qr)
}

func update(tree []int32, i int, val int32) {
	n := (len(tree) + 1) / 2
	i += n - 1
	tree[i] = int32(val)
	for i > 0 {
