...
// update устанавливает значение i-го (0-indexing) элемента
func (t *tree) update(i int, v int64) {
	delta := v - t.items[i]
	t.items[i] = v
	for i < len(t.sums) {
		t.sums[i] += delta
...
// query возвращает суммму на открытом интервале [l, r) 0-indexing
func (t *tree) query(l, r int) int64 {
	return t.get(r-1) - t.get(l-1)
}

// get возвращает сумму префикса [0, i] 0-indexing
...
func run(in io.Reader, out io.Writer) {
	...
	for i := 0; i < k; i++ {
		...
		switch op {
		case "A": // set
			i, x, err := scanTwoInt(sc)
			if err != nil {
				panic(err)
			}
			// преходим из 1-base в 0-base индексацию
			tree.update(i-1, int64(x))
			if debugEnable {
				log.Printf("set %d %d -> %v", i, x, tree)
			}
		case "Q": // get
