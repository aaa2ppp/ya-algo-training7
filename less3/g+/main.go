...
func precalc(N int) []int64 {
	n := 1
	for n < N {
		n *= 2
	}

	tree := make([]int64, n*2-1)
...
func update(tree []int64, idx int, val int64) {
	n := (len(tree) + 1) / 2
	i := idx + n - 1
	tree[i] = val
...
func query(tree []int64, ql, qr int) int64 {
	n := (len(tree) + 1) / 2
	...
	dfs = func(i, l, r int) int64 {
		...
		m := (l + r) / 2
		return dfs(i*2+1, l, m) + dfs(i*2+2, m, r)
	}
