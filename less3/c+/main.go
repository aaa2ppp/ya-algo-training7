...
func solve(nums []int64) []int64 {
	...
	var format string
	showNumsAndAns := func() {
		maxSize2 := totalMaxSize
		maxSize10 := len(strconv.Itoa((1 << maxSize2) - 1))
		format = fmt.Sprintf("%%%dd %%0%db", maxSize10, maxSize2)
		...
		if minSize == maxSize && maxSizeCount == 1 {
			...
			for ; srcCol < maxSize; srcCol++ {
				...
				for row := 0; row < len(ans); row++ { // XXX неоптимальный старт
					// TODO: здесь нужно скипать висячую строку
					if ans[row]&(int64(1)<<bitIdx) != 0 {
						continue
					}
