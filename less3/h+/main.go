...
func newQube(size int) *cube {
	sums := make([][][]int, size)
	for i := 0; i < size; i++ {
		sums[i] = makeMatrix[int](size, size)
	}
	return &cube{
		size: size,
		sums: sums,
...
func (c *cube) update(x, y, z int, delta int) {
	for x := x; x < c.size; x = x | (x + 1) {
		for y := y; y < c.size; y = y | (y + 1) {
			for z := z; z < c.size; z = z | (z + 1) {
				c.sums[x][y][z] += delta
			}
		}
	}
...
func (h *cube) query(x1, y1, z1, x2, y2, z2 int) int {
	return h.get(x2, y2, z2) -
		(h.get(x1-1, y2, z2) + h.get(x2, y1-1, z2) + h.get(x2, y2, z1-1)) +
		(h.get(x2, y1-1, z1-1) + h.get(x1-1, y2, z1-1) + h.get(x1-1, y1-1, z2)) -
		h.get(x1-1, y1-1, z1-1)
}

func (c *cube) get(x, y, z int) int {
	var sum int
	for x := x; x >= 0; x = x&(x+1) - 1 {
		for y := y; y >= 0; y = y&(y+1) - 1 {
			for z := z; z >= 0; z = z&(z+1) - 1 {
				sum += c.sums[x][y][z]
			}
		}
	}
...
func run(in io.Reader, out io.Writer) {
	...
	for {
		...
		switch m {
		case 1:
			x, y, z, k, err := scanFourInt(sc)
			if err != nil {
				panic(err)
			}
			haven.update(x, y, z, k)
			if debugEnable {
				log.Println(haven)
			}

		case 2:
			x1, y1, z1, err := scanThreeInt(sc)
			if err != nil {
				panic(err)
			}
			x2, y2, z2, err := scanThreeInt(sc)
			if err != nil {
				panic(err)
			}
			ans := haven.query(x1, y1, z1, x2, y2, z2)
		...
		default:
			panic("unknown command " + strconv.Itoa(m))
		}
	}
}
